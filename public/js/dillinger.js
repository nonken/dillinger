...
//    Notifier.showMessage(Notifier.messages.profileCleared, 1400)
  ...
  function previewMd(){
    ...
    if (profile.showHTMLSource) {
      ...
      var unmd = editor.getSession().getValue()
      md = escape(md).replace(/[\n\s]/g, function(match) {
        return htmlReplaces[match]
      })
    }
  ...
  /**
   * Toggles the HTML view (source/preview)
   *
   * @return {Void}
   */
  function toggleHTML() {
    profile.showHTMLSource = !profile.showHTMLSource
